...
export function registerPknTools(server: McpServer, env: Env): void {
  server.tool(
    ...
    {
      query: z.string().min(1).describe("Fraza wyszukiwania (zwykły tekst)."),
      language: z
        .enum(["pl", "en", "ru"])
        .default("pl")
        .describe(
          "Site language / search path: pl — /wyszukiwarka; en — /en/search; ru — /ru/poisk.",
        ),
      sort_by: z
      ...
      page: z.number().int().min(0).default(0).describe("Numer strony wyników liczony od zera (paginacja Drupala)."),
    },
