...
export function registerSumTools(server: McpServer, env: Env): void {
  ...
  server.tool(
    ...
    ].join(" "),
    {
      local_base: z
        ...
        .min(1)
        .default("SUM01")
        .describe("Kod bazy bibliograficznej Aleph, np. SUM01; sprawdź w lokalnej dokumentacji Aleph"),
      request: z
        .string()
        .min(1)
        .describe("Zapytanie find w składni prefiksów WWW, np. wrd=medycyna albo wti=anestezjologia"),
    },
  ...
  server.tool(
    ...
    {
      set_no: z.string().min(1).describe("Numer zestawu wyników zwrócony przez find, np. 000001"),
      set_entry: z
        .string()
        .min(1)
        .describe(
          "Indeks wpisu lub zakres: jedna wartość (np. 000000001) albo od-do (np. 000000001,000000005) wg dokumentacji Aleph",
        ),
      format: z.string().default("marc").describe("Format prezentacji wyników, np. marc"),
    },
