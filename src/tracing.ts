...
export async function withAgentRequestSpan<T>(
  ...
  return tracer.startActiveSpan("agent.request", { kind: SpanKind.SERVER }, async (span) => {
    try {
      ...
      span.setAttribute("agent.has_polish_chars", /[ąęóśźżćńł]/i.test(queryText));
      span.setAttribute("agent.query_token_estimate", estimateTokens(queryText));
      span.setAttribute("agent.query_is_boolean", /\bAND\b|\bOR\b|\bNOT\b/.test(queryText));
      span.setAttribute("agent.query_has_date_range", /\b\d{4}\b.*\b\d{4}\b/.test(queryText));
      span.setAttribute("agent.query_has_author", /autor|author|napisał|napisała/i.test(queryText));
      span.setAttribute("agent.query_has_subject", /temat|subject|dziedzina|UKD/i.test(queryText));
...
export async function withLlmCallSpan<T>(
  ...
  return tracer.startActiveSpan("llm.call", async (span) => {
    try {
      ...
      span.setAttribute("llm.decided_to_use_tool", meta.toolDecision !== null);
      ...
      if (meta.systemPromptTokens !== undefined) {
        span.setAttribute("llm.prompt_tokens_system", meta.systemPromptTokens);
      }
      if (meta.retrievedMetadataTokens !== undefined) {
        span.setAttribute("llm.prompt_tokens_context", meta.retrievedMetadataTokens);
      }
      if (meta.userQueryTokens !== undefined) {
        span.setAttribute("llm.prompt_tokens_query", meta.userQueryTokens);
      }
      if (meta.conversationHistoryTokens !== undefined) {
        span.setAttribute("llm.prompt_tokens_history", meta.conversationHistoryTokens);
      }
      if (
        ...
        const total = meta.systemPromptTokens + meta.retrievedMetadataTokens + meta.userQueryTokens;
        span.setAttribute(
      ...
      if (meta.priorToolsInSession !== undefined) {
        span.setAttribute("llm.prior_tools_in_session", meta.priorToolsInSession.join(","));
        if (meta.chosenTool) {
          span.setAttribute(
            "llm.repeated_tool",
...
export async function withToolSelectionSpan<T>(
  ...
  return tracer.startActiveSpan("mcp.tool_selection", async (span) => {
    try {
      ...
      span.setAttribute("mcp.selection_reason", selectionReason.slice(0, 200));
      span.setAttribute("mcp.selection_is_first_candidate", candidateTools[0] === selectedTool);
      span.setAttribute("mcp.reasoning_word_count", selectionReason.split(/\s+/).length);
      span.setAttribute(
  ...
  return tracer.startActiveSpan(`mcp.tool/${meta.toolName}`, async (span) => {
    ...
    try {
      ...
      span.setAttribute(
        ...
        typeof meta.params.language === "string" ? meta.params.language : "none",
      );
      span.setAttribute("params.filter_has_subject", meta.params.subject !== undefined);
      span.setAttribute("params.filter_has_author", meta.params.author !== undefined);
      span.setAttribute(
      ...
      span.setAttribute("params.query_is_phrase", /".+"/.test(String(meta.params.query ?? "")));
      span.setAttribute(
      ...
      span.setAttribute("fragment.fields_requested", meta.fieldsRequested.join(","));
      span.setAttribute("fragment.fields_returned", meta.fieldsReturned.join(","));
      span.setAttribute("fragment.fields_requested_n", meta.fieldsRequested.length);
      span.setAttribute("fragment.fields_returned_n", meta.fieldsReturned.length);
      const omitted = meta.fieldsRequested.filter((f) => !meta.fieldsReturned.includes(f));
      span.setAttribute("fragment.fields_omitted", omitted.join(","));
      span.setAttribute(
        "fragment.omission_rate",
        meta.fieldsRequested.length > 0 ? omitted.length / meta.fieldsRequested.length : 0,
      );
      ...
      for (const field of [
        ...
        span.setAttribute(`fragment.has_${field}`, meta.fieldsReturned.includes(field));
      }

      // Context window usage per field
      const totalTokens = Object.values(meta.tokensByField).reduce((a, b) => a + b, 0);
      span.setAttribute("context.tokens_total", totalTokens + meta.queryTokens);
      span.setAttribute("context.tokens_query", meta.queryTokens);
      span.setAttribute("context.tokens_metadata", totalTokens);
      span.setAttribute("context.tokens_title", meta.tokensByField["title"] ?? 0);
      span.setAttribute("context.tokens_author", meta.tokensByField["author"] ?? 0);
      span.setAttribute("context.tokens_subject", meta.tokensByField["subject"] ?? 0);
      span.setAttribute("context.tokens_abstract", meta.tokensByField["abstract"] ?? 0);
      span.setAttribute("context.tokens_ukd", meta.tokensByField["ukd"] ?? 0);
      span.setAttribute(
        "context.metadata_ratio",
        totalTokens + meta.queryTokens > 0 ? totalTokens / (totalTokens + meta.queryTokens) : 0,
      );
...
export function detectLanguage(text: string): "pl" | "en" | "mixed" | "unknown" {
  ...
  const enWords = (text.match(/\b(the|and|for|with|from|that)\b/gi) ?? []).length;
 ...
 */
export function detectFieldsInText(text: string, candidates: string[]): string[] {
...
export function annotateCurrentSpan(attrs: Record<string, string | number | boolean>): void {
