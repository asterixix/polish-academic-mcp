...
/** Pełny katalog narzędzi dla gościa (IP) i OAuth access_token — zgodny z rejestracją w src/tools/*.ts. */
...
const handler = {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    // OAuth 2.1 AS + RFC 7591 DCR (public POST /register, known MCP redirect URIs merged).
        ...
        if (
          ...
          return new Response(
            JSON.stringify({
              ...
              message: `Tool '${toolCallName}' is not in the public tool set for this connection. Use a Connect JWT from /admin/tokens with allowed_tools including this name, or ask the operator to add it to the worker public catalog.`,
            }),
            {
              status: 403,
              headers: { "Content-Type": "application/json" },
