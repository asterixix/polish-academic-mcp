...
export interface ToolResponse {
  raw: unknown;
  text: string;           // stringified for text analysis
  latencyMs: number;
  statusCode: number;
 ...
 */
export function scoreContextFillRatio(
  ...
  return {
    ...
    notes: `Context fill ratio: ${(ratio * 100).toFixed(1)}%. ${
      ratio > 0.9 ? "WARNING: metadata dominates — query may be lost." :
 ...
 */
export function scoreFragmentOmission(
  response: ToolResponse,
  testCase: EvalTestCase,
): MetricScore {
  const span = response.spanAttributes ?? {};
 ...
 */
export function scoreClassificationDrift(
  response: ToolResponse,
  testCase: EvalTestCase,
): MetricScore {
  const span = response.spanAttributes ?? {};
 ...
 */
export function scoreLanguageQuality(
  response: ToolResponse,
  testCase: EvalTestCase,
): MetricScore {
  const span = response.spanAttributes ?? {};
 ...
 */
export function scoreSemanticShift(
  response: ToolResponse,
): MetricScore {
  const span = response.spanAttributes ?? {};
 ...
 */
export function scoreGroundTruthFidelity(
  response: ToolResponse,
  testCase: EvalTestCase,
): MetricScore {
 ...
 */
export function scoreCataloguingCompleteness(
  response: ToolResponse,
  testCase: EvalTestCase,
): MetricScore {
  const span = response.spanAttributes ?? {};
 ...
 */
export function scorePiiExposure(
  response: ToolResponse,
  testCase: EvalTestCase,
): MetricScore {
 ...
 */
export function scoreAuditTrail(
  response: ToolResponse,
): MetricScore {
  const span = response.spanAttributes ?? {};
 ...
 */
export function scoreDataMinimisation(
  response: ToolResponse,
  testCase: EvalTestCase,
): MetricScore {
  const span = response.spanAttributes ?? {};
  ...
  const requiresPersonalData = testCase.requiredFields.some((f) =>
    ["author", "orcid", "email", "affiliation"].includes(f),
  );
 ...
 */
export function scoreAiTransparency(
  response: ToolResponse,
  testCase: EvalTestCase,
): MetricScore {
