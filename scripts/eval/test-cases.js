...
const TOOL_ARGS = {
    ...
    eval_response: {
        ...
        generated_text: "Artykuł autorstwa Jana Kowalskiego dotyczy zastosowania uczenia maszynowego w bibliotekach cyfrowych.",
...
// ─────────────────────────────────────────────────────────────────────────────
function rq1Desc(m) {
    return (`[RQ1] ${m.label}: measures context fill ratio, fragment omission rate, ` +
        `tool selection accuracy (M3), latency (≤5 s), and token efficiency for this tool's response payload.`);
}
function rq2Desc(m) {
    return (`[RQ2] ${m.label}: checks metadata fidelity — hallucination detection, ` +
...
function rq3Desc(m) {
    return (`[RQ3] ${m.label}: evaluates attribution transparency, tool-reasoning ` +
        `auditability, and cataloguing completeness — the librarian epistemic responsibility dimension.`);
}
function rq4Desc(m) {
    return (`[RQ4] ${m.label}: tests GDPR/EU AI Act alignment — PII exposure, audit trail ` +
        `completeness, data minimisation preference, and AI transparency markers.`);
}
// ─────────────────────────────────────────────────────────────────────────────
// Case factory — generates one EvalTestCase per (tool, RQ)
// ─────────────────────────────────────────────────────────────────────────────
function makeCase(rq, idx, tool) {
    const m = TOOL_META[tool];
    if (!m)
        throw new Error(`No TOOL_META entry for tool: ${tool}`);
    const args = TOOL_ARGS[tool];
    if (!args)
        throw new Error(`No TOOL_ARGS entry for tool: ${tool}`);
    const pad = String(idx).padStart(3, "0");
    const id = `${rq}-T${pad}`;
    const descFn = rq === "RQ1" ? rq1Desc :
        rq === "RQ2" ? rq2Desc :
            rq === "RQ3" ? rq3Desc :
                rq4Desc;
    return {
...
// ─────────────────────────────────────────────────────────────────────────────
export const RQ1_CASES = ALL_TOOLS.map((tool, i) => makeCase("RQ1", i + 1, tool));
export const RQ2_CASES = ALL_TOOLS.map((tool, i) => makeCase("RQ2", i + 1, tool));
export const RQ3_CASES = ALL_TOOLS.map((tool, i) => makeCase("RQ3", i + 1, tool));
export const RQ4_CASES = ALL_TOOLS.map((tool, i) => makeCase("RQ4", i + 1, tool));
// ─────────────────────────────────────────────────────────────────────────────
// Legacy placeholder — kept so old imports don't break
// ─────────────────────────────────────────────────────────────────────────────
/** @deprecated Use ALL_TEST_CASES — EXTENDED_CASES is now empty. */
export const EXTENDED_CASES = [];
// ─────────────────────────────────────────────────────────────────────────────
// Combined export
// ─────────────────────────────────────────────────────────────────────────────
...
export function getCasesByRQ(rq) {
    return ALL_TEST_CASES.filter((c) => c.rq.includes(rq));
}
export function getCasesByTool(tool) {
 ...
 */
export function toolForEvalResponseCompositeScore(testCase) {
    return testCase.tool === "eval_response" ? "eval_response" : testCase.tool;
}
export function getCasesBySchema(schema) {
